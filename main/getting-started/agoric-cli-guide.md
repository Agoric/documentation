...
1. [Starting Multiuser Daps](#starting-multiuser-dapps): How to use the Agoric CLI to deploy and start a local-chain-multiuser scenario, for development and and testing of multiuser Dapps
...
- **Function**:
  ...
  The reason is that there is both an SDK and npm mode. Currently we only support SDK mode, which allows you to link your Dapp 
  against the SDK dependencies. This lets you to modify the SDK without having to publish it to npm.
- **Required Arguments**:
  - None.
- **Optional Arguments**:
  - None.
