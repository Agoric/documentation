...
To get a reference to an invitation for creating a new pool, you need use the `addPoolInvitation()`
method which is exposed by the AMM's publicFacet.
...
amount of the central token in the offer. Keep in mind that when funding a new pool, there is
a minimum liquidity requirement that must be met, or else the offer will get rejected.
