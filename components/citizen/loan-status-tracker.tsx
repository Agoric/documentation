...
import { CheckCircle, Clock, AlertCircle, FileText, CreditCard, Home, ArrowRight, RefreshCw } from "lucide-react"
...
export function LoanStatusTracker({ loanId, compact = false, onStepComplete }: LoanStatusTrackerProps) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'under-review': return <RefreshCw className="h-5 w-5 text-yellow-500" />
      case 'approved': return <CheckCircle className="h-5 w-5 text-green-500" />
      case 'funded': return <CheckCircle className="h-5 w-5 text-emerald-500" />
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'funded': return 'bg-emerald-500'
  ...
  const getLoanTypeIcon = (type: string) => {
    switch (type) {
      case 'Home Mortgage': return <Home className="h-5 w-5" />
      case 'Auto Loan': return <CreditCard className="h-5 w-5" />
  ...
  return (
    <Card className="bg-gradient-to-br from-blue-900/50 to-cyan-\
