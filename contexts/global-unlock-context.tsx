...
export function GlobalUnlockProvider({ children }: { children: React.ReactNode }) {
  ...
  const [unlockedSuggestions, setUnlockedSuggestions] = useState<string[]>([])
  ...
  const getSuggestionStatus = (suggestionId: string) => {
    if (isEverythingUnlocked) return "unlocked"
    return unlockedSuggestions.includes(suggestionId) ? "unlocked" : "locked"
  }
