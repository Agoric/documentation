...
export async function GET(request: NextRequest) {
  try {
    ...
    // Downgrade to warn so it does not look like a hard-failure in the console
    if (!(error instanceof ZillowSubscriptionError)) {
      console.warn("Zillow API Error:", error)
    }
    ...
    return NextResponse.json(
      {
        ...
        message: isSubError
          ? "Live Zillow data unavailable – subscription required. Showing demo data."
